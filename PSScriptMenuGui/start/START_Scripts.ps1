...
Remove-Module ZK -ErrorAction SilentlyContinue
[System.Console]::Title = "РТК-IT $env:computername | $env:USERDNSDOMAIN | GUI-PowerShell"

try {
    Import-Module MenuGui -ErrorAction Stop
}
catch {
    Write-Warning $_
    Write-Verbose 'Попытка импорта из родительского каталога...' -Verbose
    Import-Module '..\'
}
#endregion

$params = @{
    csvPath = '.\rtk_info.csv'
    buttonForegroundColor = 'Azure'
    buttonBackgroundColor = '#007bc7'
